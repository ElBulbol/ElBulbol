...
export const projects = [
  {
    ...
    attacks: [
      { name: 'SQL Injection', payload: "' OR 1=1 --", description: 'Raw string formatting in login and search queries allows arbitrary SQL execution.' },
      { name: 'IDOR', payload: '/property/42', description: 'Property and admin routes accept direct object IDs without authorization checks.' },
      { name: 'XSS', payload: '<script>alert(1)</script>', description: 'Unescaped user input rendered directly in HTML templates.' },
      { name: 'CSRF', payload: null, description: 'Forms lack anti-CSRF tokens, enabling cross-site request forgery.' },
      { name: 'Plaintext Credentials', payload: null, description: 'Passwords stored and compared in plaintext, with no hashing.' },
    ],
    defense: [
      { name: 'ModSecurity', description: 'Web application firewall with OWASP Core Rule Set that blocks SQLi, XSS, and malicious payloads at the HTTP layer.' },
      { name: 'Suricata IDS/IPS', description: 'Network-level intrusion detection and prevention that inspects traffic for known attack signatures.' },
      { name: 'pfSense', description: 'Network firewall that controls traffic flow and enforces network-level access policies.' },
    ],
    certification: { name: 'HCIA-Security', issuer: 'NTI', date: 'February 2025', link: null },
    screenshots: null,
    story: null,
  },
  {
